...
ice = 1.0 - r/(r_soe_ini)  # iceberg cost

# TFP growth
g_t = 0.038  # exogenous TFP growth
bet = bet * (1+g_t)**(1.0-sig)  # TFP growth adjusted discount factor
bet_E = bet_E * (1+g_t)**(1.0-sig)  # TFP growth adjusted discount factor

# calibration targets
# the ratio of K/Y in the F sector to K/Y in the E sector
KY_F_E = 2.65
# the ratio of the rate of return in the E sector to that in the F sector
rho_r = (r_soe_ini+0.09) / (r/(1.0-ice))
# share of managerial compensation
psi = 1.0 - (rho_r*r/(1.0-ice)+delta)/(r/(1.0-ice)+delta)/KY_F_E
# productivity ratio of E over F
ksi = (KY_F_E)**(alp/(1-alp)) / (1.0-psi)

# bank loan in the E sector
loan_asset = 1.0  # loan asset ratio
# measure of financial frictions
eta = loan_asset * (1+r/(1-ice)) / (
...
if g_n > 0 or g_n < 0:
    ...
    sum_demo = (1.0-(1.0+g_n)**age_max) / (1.0-(1.0+g_n))
    ...
    for i in range(age_max):
        n_weight[i] = 1.0 / age_max * n_pre
        e_weight[i] = 1.0 / age_max * e_pre
...
for t in range(time_ab - 1, time_cd - 1):
    ice_t[t] = ice + ((t-(time_ab-1))**speed
                      * (ice_t[time_cd-1]-ice) / (time_cd-time_ab)**speed)

# check parameter values
print('Assumption 1: > 0')
print(ksi - (1.0/(1.0-psi))**(1.0/(1.0-alp)))

###############################################################################
#                            pre_transition.m                                 #
###############################################################################

# total capital during pre-transition period (all in the F sector)
k_pre = (alp/(r/(1.0-ice)+delta))**(1.0/(1.0-alp)) * nw_pre
# wage rate during pre-transition period
w_pre = (1.0-alp) * (alp/(r/(1.0-ice)+delta))**(alp/(1.0-alp))
...
def saving_pre_transition(age, wage, wealth):
    ...
    for i in range(age, age_max+1):
        if i < age_T_w:
            w[i-1] = wage * ((1.0+g_t)/(1+r))**(i-age)  # earnings
        else:
            w[i-1] = 0.0
    # Life-time wealth (evaluated at t+1)
    A = np.sum(w) + wealth*(1.0+r)
    ...
    for i in range(age, age_max + 1):
        ...
        if i == age:
            ...
            ratio[i-1] = ((bet*(1.0+r)/(1.0+g_t))**(1.0/sig)
                          * (1.0+g_t) / (1.0+r)
                          * ratio[i-2])
    ...
    wealth_prime = (wealth*(1.0+r)+wage-consumption) / (1.0+g_t)
...
def saving_pre_transition_E(age, wage, wealth):
    ...
    for i in range(age, age_max+1):
        if i < age_T_w:
            w[i-1] = wage * ((1.0+g_t)/(1.0+r))**(i-age)  # earnings
        else:
            w[i-1] = 0.0
    # Life-time wealth (evaluated at t+1)
    A = np.sum(w) + wealth*(1.0+r)
    ...
    for i in range(age, age_max+1):
        ...
        if i == age:
            ...
            ratio[i-1] = ((bet_E*(1.0+r)/(1.0+g_t))**(1.0/sig)
                          * (1.0+g_t) / (1.0+r)
                          * ratio[i-2])
    # optimal consumption and savings
    consumption = A / (np.sum(ratio))
...
for i in range(age_max):
    ...
    (_,
     ...
     sr_pre[i],
     consumption_pre[i]
     ) = saving_pre_transition(age, wage, wealth)
    if age < age_max:
...
for i in range(age_max):
    ...
    (_,
     ...
     sr_pre_E[i],
     consumption_pre_E[i]
     ) = saving_pre_transition_E(age, wage, wealth)
    if age < age_max:
...
def saving_E_existing(age, current_wealth):
    ...
    rho_t_ad = np.maximum(
            rho_t,
            (
             (rho_t*(1.0+r/(1.0-ice_t))+eta*(rho_t-r/(1.0-ice_t)))
             / (1.0+r/(1.0-ice_t)-eta*(rho_t-r/(1.0-ice_t)))
            )
            )
    for i in range(age, age_max+1):
        if i < age_T:
            w[i-1] = m_t[i-age]*((1.0+g_t)/(1.0+r))**(i-age)  # earnings
        else:
            w[i-1] = 0.0
    if age < age_T:
        A = np.sum(w)+(1.0+r)*wealth[age-1]  # Check how wealth is defined
    else:
        A = np.sum(w)+(1.0+rho_t_ad[0])*wealth[age-1]
    ...
    for i in range(age, age_max+1):
        ...
        if i == age:
            ...
            ratio[i-1] = ((bet_E*(1+r)/(1.0+g_t))**(1.0/sig)
                          * (1.0+g_t) / (1.0+r)
                          * ratio[i-2])
        else:  # become firm owner
            ratio[i-1] = ((bet_E*(1.0+rho_t_ad[i-age])/(1.0+g_t))**(1.0/sig)
                          * (1.0+g_t) / (1.0+rho_t_ad[i-age])
                          * ratio[i-2])
    ...
    for i in range(age, age_max+1):
        if i == age:
            ...
            consumption[i-1] = (
                    (bet_E*(1.0+rho_t_ad[i-age])/(1.0+g_t))**(1.0/sig)
                    * consumption[i-2]
                    )
            if i < age_max:
                wealth[i] = (
...
def saving_E_newly_born(year_birth):
    ...
    for i in range(1, age_max+1):
        if i < age_T:
            w[i-1] = m_t[tt+i-2] * ((1.0+g_t)/(1.0+r))**(i-1)  # earnings
        else:
            w[i-1] = 0.0
    A = np.sum(w)
    ...
    for i in range(1, age_max+1):
        ...
        if i == 1:
            ...
            ratio[i-1] = (
                    (bet_E*(1.0+r)/(1.0+g_t))**(1.0/sig)
                    * (1.0+g_t) / (1.0+r) * ratio[i-2]
                    )
        else:  # become firm owner
            ratio[i-1] = (
                    (bet_E*(1.0+rho_t_ad[tt+i-2])/(1.0+g_t))**(1.0/sig)
                    * (1.0+g_t) / (1.0+rho_t_ad[tt+i-2]) * ratio[i-2]
                    )
    ...
    for i in range(1, age_max+1):
        if i == 1:
            consumption[i-1] = A / (np.sum(ratio))
            wealth[i] = (m_t[tt-1]-consumption[i-1]) / (1.0+g_t)
        elif i < age_T:  # being manager
            consumption[i-1] = (
                    (bet_E*(1.0+r)/(1.0+g_t))**(1.0/sig) * consumption[i-2]
                    )
            wealth[i] = (
                    (wealth[i-1]*(1.0+r)+m_t[tt+i-2]-consumption[i-1])
                    / (1.0+g_t)
                    )
        else:  # become firm owner
            consumption[i-1] = (
            ...
            if i < age_max:
                wealth[i] = (
                        (wealth[i-1]*(1.0+rho_t_ad[tt+i-2])-consumption[i-1])
                        / (1.0+g_t)
                        )
    return wealth, consumption
...
while dev_max > tol and iteration < iter_max:
    ...
    for t in range(time_max):
        ...
        for i in range(age_max):
            ...
            AE[t, i] = e_weight[i] * ae[t, i]

        # capital and labor in the E sector
        for i in range(age_T-1, age_max):
            if rho_t[t] >= r / (1.0-ice_t[t]):  # borrowing is profitable
                loan_ratio[t] = (
                        eta * (1.0+rho_t[t])
                        / (
                                1.0 + r/(1.0-ice_t[t])
                                - eta*(rho_t[t]-r/(1.0-ice_t[t]))
                        )
                        )  # loan asset ratio
        ...
        # wage rate
        w_t_new[t] = (
                (1.0-psi) * (1.0-alp) * (KE_t[t]/NE_t[t])**alp
                * ksi**(1.0-alp)
                )
        ...
        if I_end == 0:
            w_t_new[t] = (
                    (1.0-alp) * (alp/(r/(1.0-ice_t[t])+delta))**(alp/(1.0-alp))
                    )  # wage rate
        else:
            NE_t[t] = N_t[t]
            w_t_new[t] = (
                    (1.0-psi) * (1.0-alp) * (KE_t[t]/N_t[t])**alp
                    * ksi**(1.0-alp)
                    )  # wage rate
        ...
        rho_t_new[t] = np.max(
                [r,
                 (
                         (1.0-psi)**(1.0/alp) * ksi**((1.0-alp)/alp)
                         * ((1.0-alp)/w_t_new[t])**((1.0-alp)/alp) * alp
                         - delta
                 )
                 ]
                )
        # aggregate output in the E sector
        YE_t[t] = KE_t[t]**alp * (ksi*NE_t[t])**(1.0-alp)
    ...
    # renew
    w_t = w_t*relax + w_t_new*(1.0-relax)
    rho_t = rho_t*relax + rho_t_new*(1.0-relax)
    m_t = m_t*relax + m_t_new*(1.0-relax)
...
def saving_F_existing(age, current_wealth):
    ...
    for i in range(age, age_max+1):
        if i < age_T_w:
            w[i-1] = w_t[i-age] * ((1.0+g_t)/(1.0+r))**(i-age)  # earnings
        else:
            w[i-1] = 0.0
    ...
    for i in range(age, age_max+1):
        ...
        if i == age:
            ...
            ratio[i-1] = (
                    (bet*(1.0+r)/(1.0+g_t))**(1.0/sig)*(1.0+g_t)/(1.0+r)
                    * ratio[i-2]
                    )
    ...
    for i in range(age, age_max+1):
        if i == age:
            ...
            if i < age_T_w:
                wealth[i] = (
                        (wealth[i-1]*(1.0+r)+w_t[i-age]-consumption[i-1])
                        / (1+g_t)
                        )
            elif i < age_max:
                wealth[i] = (wealth[i-1]*(1.0+r)-consumption[i-1]) / (1.0+g_t)
        elif i < age_T_w:  # being worker
            consumption[i-1] = (
                    (bet*(1.0+r)/(1.0+g_t))**(1.0/sig)*consumption[i-2]
                    )
            wealth[i] = (
                    (wealth[i-1]*(1.0+r)+w_t[i-age]-consumption[i-1])
                    / (1.0+g_t)
...
def saving_F_newly_born(year_birth):
    ...
    for i in range(1, age_max+1):
        if i == 1:
            consumption[i-1] = A / (np.sum(ratio))
            wealth[i] = (w_t[tt-1]-consumption[i-1]) / (1.0+g_t)
        elif i < age_T_w:  # being workers
            consumption[i-1] = (
                    (bet*(1.0+r)/(1.0+g_t))**(1.0/sig) * consumption[i-2]
                    )
            wealth[i] = (
                    (wealth[i-1]*(1.0+r)+w_t[tt+i-2]-consumption[i-1])
                    / (1.0+g_t)
                    )
        else:  # become retirees
            consumption[i-1] = (
...
for t in range(time_max):
    ...
    if NE_t[t] < N_t[t]:
        KF_t[t] = (
                (alp/(r/(1.0-ice_t[t])+delta))**(1.0/(1.0-alp))
                * (N_t[t]-NE_t[t])
                )  # aggregate capital in the F sector
        YF_t[t] = (
                KF_t[t]**alp * (N_t[t]-NE_t[t])**(1.0-alp)
...
for t in range(time_max-1):
    ...
    IF_t[t] = (1.0+g_t)*(1.0+g_n)*KF_t[t+1] - (1.0-delta)*KF_t[t]

    # computing investment in the E sector
    IE_t[t] = (1.0+g_t)*(1.0+g_n)*KE_t[t+1] - (1.0-delta)*KE_t[t]
    ...
    SF_t[t] = (1.0+g_t)*(1.0+g_n)*AF_t[t+1] - AF_t[t] + delta*KF_t[t]
    if YF_t[t] > 0:
        SF_YF_t[t] = SF_t[t] / YF_t[t]

    # computing enterpreneurs' savings
    SE_t[t] = (1.0+g_t)*(1.0+g_n)*AE_t[t+1] - AE_t[t] + delta*KE_t[t]
    ...
    if t > 0:
        TFP_t[t] = (
                Y_t[t]/Y_t[t-1]
                - alp*K_t[t]/K_t[t-1]
                - (1.0-alp)*N_t[t]/N_t[t-1]
                )
        YG_t[t] = (Y_t[t]/Y_t[t-1]-1.0) + g_n + g_t
